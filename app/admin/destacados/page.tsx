...
export default async function AdminDestacadosPage() {
  ...
  const [{ data: posts, error: postsError }, { data: settings, error: settingsError }] =
    await Promise.all([
      supabase
        .from("media")
        .select("*")
      ...
      supabase.from("feature_settings").select("destacados_enabled").eq("id", true).maybeSingle(),
    ]);
  ...
  return (
    <main className="mx-auto w-full max-w-4xl flex-1 px-4 py-6">
      <AdminNav />
      <DestacadosScreen items={items} destacadosEnabled={settings?.destacados_enabled ?? false} />
    </main>
  );
}
